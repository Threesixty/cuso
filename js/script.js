    ...
    var App = {
        ...
        header: function () {
            ...
            var header = $('.header'),
                ...
                subnavToggle = $('.header-subnav--toggle'),
                subnavTimer;
            if (subnav.length) {
            ...
            var listContainer = $('.header-link'),
                listItem = '.header-list > li',
                listClass = 'header-list';
            if (listContainer.length) {
                listContainer.each(function () {
                    var listColumn = $(this).data('column') !== undefined ? $(this).data('column') : 2;
                    var container = $(this),
                        items = container.find(listItem),
                        itemsPerColumn = new Array([]),
        ...
        wizard: function () {
            ...
            if ($('.wizard').length) {
                initSelect2();
                $('.wizard-layer .next, .wizard-layer .prev').on('click', function() {
                    if ($(this).hasClass('next')) {
                        nextBtnClicked = true;
                        var form = $(this).closest('form'), 
                            yiiActiveFormData = form.data('yiiActiveForm');

                        $('.wizard-layer.active').find('input, select, checkbox').each(function() {
                            if (undefined === $(this).attr('id'))
                                form.yiiActiveForm('validate');
                            form.yiiActiveForm('validateAttribute', $(this).attr('id'));
                        });
                    } else {
                        $('.wizard-layer').removeClass('active');
                        $('#layer'+$(this).data('id')).addClass('active');
                ...
                $('.wizard-layer .image-box').on('click', function() {
                    $('.wizard-layer .file-input input').trigger('click');
                });

                $('.wizard-layer .file-input input').on('change', function() {
                    var currentInput = $(this);
                    if ($(this).prop('files') && $(this).prop('files')[0]) {
                        var reader = new FileReader();
                        reader.readAsDataURL($(this).prop('files')[0]);
                        reader.onload = function (e) {
                            currentInput.closest('.file-input').find('.image-box').attr('style', 'background-image:url(' + e.target.result + ')')
                        };
                    }
                });
            }
            ...
            $('#register-form').on('afterValidate', function (event, messages, errorAttributes) {
                if (nextBtnClicked) {
                    ...
                    for (const [key, value] of Object.entries(messages)) {
                        if (value.length > 0 && $('#'+key).closest('.no-company').css('display') != 'none') {
                            fieldsOk = false;
                        }
                    }
                    if (fieldsOk) {
                        var nextId = $('.wizard-layer.active').find('.next').attr('data-id');
                        $('.wizard-layer').removeClass('active');
                        $('#layer'+nextId).addClass('active');
            ...
            function initSelect2() {
                $('.wizard-layer.active').find('select').each(function() {
                    if (!$(this).hasClass('select2')) {

                        $(this).addClass('select2');

                        var select2Handler = $(this).select2({
                        ...
                        .on('select2:open', (elm) => {
                            const targetLabel = $(elm.target).closest('.form-group').find('label');
                            targetLabel.addClass('selected');
                        })
                        .on('select2:close', (elm) => {
                            const targetLabel = $(elm.target).closest('.form-group').find('label');
                            const targetOptions = $(elm.target.selectedOptions);
                            if ($(targetOptions[0]).attr('value') == '') {
                                targetLabel.removeClass('selected');
                            }
                        });

                        $('.select-company').on('select2:select', $(this), function(e) {
                            $('.no-company').hide();
                        });
            ...
            $('.show-company-form').on('click', function() {
                $('.no-company').slideToggle();

                $('.wizard-layer.active').find('.select2').val(null).trigger('change');
            });
        }
