...
$baseUrl = str_replace('backend', '', Url::base(true)); ?>
    ...
    <div class="col-lg-12 block <?= !$show ? 'block-widget' : 'init-summernote' ?> block-partners my-10" data-block="partners" data-init="summernote" <?= $show ? 'data-idx="'.($idx+1).'"' : '' ?>>
        ...
        <div class="block-unit bg-gray-100 p-5 border-light-dark rounded">
            <h3 class="text-primary text-uppercase">Partenaires</h3>
            ...
            <div class="form-group">
                ...
                <div class="mt-3 text-center card-body rounded border-light-dark p-0 content-media">
                    <input type="hidden" name="logos" value="<?= isset($dataArr['logos']) ? $dataArr['logos'] : '[]' ?>">
                    <div class="row draggable-zone draggable-max p-5">
                        <p class="content-photo-msg m-5 text-center" <?= isset($dataArr['logos']) && $dataArr['logos'] != '[]' ? 'style="display: none"' : '' ?>>Glisser-déposer une photo provenant de la bibliothèque</p>

                        <?php
                        if (isset($dataArr['logos'])) {
    
                            foreach (JSON::decode($dataArr['logos']) as $photoId) {
