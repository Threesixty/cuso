...
$dataArr = null !== $value ? JSON::decode($value) : []; ?>
    ...
    <div class="col-lg-12 block <?= !$show ? 'block-widget' : '' ?> block-all-events my-10" data-block="all-events" data-init="select2" <?= $show ? 'data-idx="'.($idx+1).'"' : '' ?>>
        ...
        <div class="block-unit bg-gray-100 p-5 border-light-dark rounded">
            <h3 class="text-primary text-uppercase">Tous les événements</h3>
        </div>
    </div>