...
var KTApp = function() {
    ...
    var initTreeVariation = function() {

        if ($('.tree-variation').length) {
                                        ...
                                        "add" : {
                                            ...
                                            "action"            : function (data) {
                                                ...
                                                if ($('.variation-' + obj.id).length) {
                                                    ...
                                                    $('.variation-container .d-none').clone().appendTo('.variation-container').removeClass('d-none').addClass('variation-' + obj.id).attr('data-id', obj.id).attr('data-name', text).find('h3 > span').html(text);
                                                    if ($('.variation-' + obj.id + ' .draggable-zone').length) {
                                        ...
                                        "delete" : {
                                            ...
                                            "action"            : function (data) {
                                                ...
                                                if ($('.variation-' + obj.id).length) {
                                                    ...
                                                    var jsonContent = JSON.parse($('.tree-variation').closest('.card-body').find('input').val());
                    ...
                    case 'rename_node':
                        if ($('.variation-' + data.node.id).length)
                            ...
                            for (var idx=0; idx < jsonContent.length; idx++) {
                                ...
                                for (key in jsonContent[idx]) {
                                    if (key == 'id' && jsonContent[idx][key] == data.node.id) {
                                    	jsonContent[idx]['name'] = data.text;
                                    }
                                }
                            }
                            jsonContent = jsonContent.filter(function(data) { return data !== null }); 
    ...
    var initTreeContent = function() {
        ...
        if ($('.tree-content').length) {
                    ...
                    "core" : {
                        ...
                        "check_callback" : function(operation, node, parent, position, more) {
                            if (more !== undefined && more.ref != undefined) {
                                if (operation == 'move_node') {
                                    if (parent.id != '#' && ((parent.id != 'j1_1' && node.parent != '#' && node.parent != 'j1_1') || node.type == 'file')) {
                                        return true
                                    } else {
                                        return false;
                                    }
                        ...
                        'data': [
                            {
                                ...
                                "children": JSON.parse($(this).prev().val()),
                            }
                        ]
                    },
                    "plugins" : ["dnd", "contextmenu", "types", "languages"],
                            ...
                            if ($node.parent != '#' && $node.parent != 'j1_1' && $node.type != 'file') {
                                ...
                                return {
                                    "id" : {
                                        ...
                                        "separator_after"   : true,
                                        "_disabled"         : false,
                                        "label"             : '<strong><p class="fas fa-hashtag" style=""></p>' + $node.data.id + '</strong>',
                                        "action"            : function (data) {
                                            $('.dataTables_filter input').val($node.data.id);
            ...
            $(document).on('click', '.add-to-menu', function() {
                ...
                var obj = {
                        'text': parent.find('.h6 strong').text(),
                        'type': 'file',
                        "data": {
                            'id': parent.find('td:first-child').text(),
                        }
                    };

                $("#menusTree").jstree('create_node', '#j1_1', obj, 'first', function (node) {
    ...
    var initTreeFaq = function() {

        var treeFaq = $('.block-faq:not(.block-widget) .tree-faq');
        if (treeFaq.length) {

            treeFaq.jstree('refresh');
            var treeObj = JSON.parse(treeFaq.prev().val());
            treeFaq.jstree({
                "core" : {
                    "themes" : {
                        "responsive": false
                    ...
                    'data': [
                        {
                            "text": treeFaq.data('name'),
                            "state": {
                                "opened": true
                            },
                            "children": treeObj[0] != undefined ? JSON.parse(treeFaq.prev().val())[0].children : [],
                        }
                    ]
                },
                "plugins" : ["dnd", "types", "languages"],
                "types" : {
                    "default" : {
                        "icon" : "fa fa-folder text-primary"
                    },
            ...
            treeFaq.on('move_node.jstree create_node.jstree redraw.jstree', function(e, data) {

                var jsonData = data.instance.get_json()[0];

                jsonData = stripJSON(jsonData);
                treeFaq.prev().val(JSON.stringify([jsonData]));
                treeFaq.closest('.form-group').find('.code-faq').html(JSON.stringify([jsonData], undefined, 4)).show();
            });
    ...
    var initFaqBlock = function(onLoad = false) {

        $(document).on('select2:select', '.block-faq select', function(e) {

            var emptySelect = [];
            $(this).closest('.block-unit').find('select').each(function() {
                if ($(this).val() != '') {
                    emptySelect[$(this).attr("name")] = $(this).val();
                    emptySelect['length'] += 1;
                }
            });

            if (emptySelect.length == 2) {

                if (e.params.load === undefined)
                    initTreeFaq();

                var currentFaq = JSON.parse($(this).closest('.block-unit').find('input[name="questions"]').val());

                $.ajax({
                    type: "POST",
                    url: $(this).closest('.block-unit').data('faq-url'),
                    data: {
                        hotel: emptySelect['hotel'],
                        category: emptySelect['category'],
                        lang: $(this).closest('.block-unit').data('lang'),
                    },
                    success: function (data) {
                        if (data) {
                            ...
                            for (var i = data.length - 1; i >= 0; i--) {
                                ...
                                var result = currentFaq[0] === undefined ? undefined : currentFaq[0].children.find(item => item.data.id === data[i].id); // item doesn't exist
                                if (result === undefined) {
                                    var obj = {
                                            text: data[i].title,
                                            type: "file",
                                    ...
                                    var createNode = $('.block-faq:not(.block-widget) .tree-faq').jstree('create_node', $('#j1_1'), obj, 'last', function (node) {
                                        if (e.params.load !== undefined)
                                            $('#'+node.a_attr.id).append('<span class="label label-lg label-danger label-pill label-inline ml-2">New</span>')
                                    });
        ...
        if (onLoad && $('.block-faq:not(.block-widget) .tree-faq').jstree(true)) {
            setTimeout(() => {
                $('.block-faq select[name="hotel"]').trigger({
                    type: 'select2:select',
                    params: {
                        load: true
                    }
    ...
    var initDatepicker = function() {
        ...
        $('.dt-end').on('change.datetimepicker', function (e) {
            $('.dt-start').datetimepicker('maxDate', e.date);
        });

        $('.t-start').datetimepicker({
            locale: 'fr',
            format: 'LT',
        });
        $('.t-end').datetimepicker({
            locale: 'fr',
            format: 'LT',
        });
    ...
    var initModal = function() {
        ...
        $(document).on('click', '.list-delete', function() {
            $('#deleteModal :submit').val($(this).closest('tr').find('td:first-child').text())
        });

        $(document).on('click', '.delete-media', function() {
            $('#deleteModal :submit').val($(this).attr('data-media-id'));
        });
    };

    var initMediaOpt = function() {
        ...
        $(document).on('click', '.show-media', function() {
            $('.sidebar-left').toggleClass('sidebar-fixed');
            $('.show-media').toggleClass('btn-light-success btn-success pulse-success pulse-light-success');
        });

        $(document).on('click', '.edit-media', function() {

            if ($(this).hasClass('no-img')) {
                $('#modalEditMedia img').closest('.overflow-image').hide();
            } else {
                $('#modalEditMedia img').closest('.overflow-image').show();
            } 
            $('#modalEditMedia input[name=media_id]').val($(this).data('media-id'));

            var extension = $(this).data('media-src').split('.');
            ...
            switch (extension) {
        		...
        		case 'mp4': 
            		$('#modalEditMedia img').addClass('d-none');
            		$('#modalEditMedia video').removeClass('d-none');//.find('source').attr('src', $(this).data('media-src'));
            		var video = $('#modalEditMedia video')[0];
                    if (video.lastChild != undefined)
                        video.removeChild(video.lastChild);
					var source = document.createElement('source');

					source.setAttribute('src', $(this).data('media-src'));
            ...
            $('#modalEditMedia .media-url code').text($(this).data('media-src'));
            $('#modalEditMedia .media-url').attr('href', $(this).data('media-src'));

            // Get media
            $.ajax({
                type: "POST",
                url: $(this).data('getmedia-url'),
                data: {id: $(this).data('media-id')},
                success: function (data) {
                    if (data) {
                        var data = JSON.parse(data);
                        $('#modalEditMedia input[name="media_title"]').val(data.title);
                        $('#modalEditMedia input[name="media_alt"]').val(data.alt);
                        $('#modalEditMedia textarea[name="media_legend"]').val(data.legend);
                        $('#modalEditMedia input[name="media_path"]').val(data.path);
                        $('#modalEditMedia select[name="media_tags[]"]').val(data.tags);
                        $('#modalEditMedia input[name="media_link"]').val(data.link);
                        $('#modalEditMedia input[name="media_lang"]').val(data.lang);

                        var tags = JSON.parse(data.tags);
                        $('#modalEditMedia input[name="media_tags[]"]').each(function(){
                        	$(this).prop('checked', false);
                        	if (null !== tags && tags.includes($(this).val()))
        ...
        $(document).on('click', '.save-media-infos', function() {
        	...
        	for (var i = 0; i < serializedForm.length; i++) {
        		if (serializedForm[i].name == 'media_tags[]') {
        			if (formData['media_tags'] === undefined)
        				formData['media_tags'] = [];
        			formData['media_tags'].push(serializedForm[i].value);
        		}
        		else
        			formData[serializedForm[i].name] = serializedForm[i].value;
        	}
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        var data = JSON.parse(data);
                        $('.media'+data.id).attr('data-original-title', data.title);
	                    ...
	                    setTimeout(function() {
	                        KTApp.block('.blockui_card', {
	                            ...
	                            opacity: '0.3',
	                            state: 'dnone',
	                            message: 'Erreur : veuillez contacter le support'
	                        });
	                        setTimeout(function() {
	                            KTApp.unblock('.blockui_card');
	                        }, 1000);
    ...
    var initMediaLang = function() {
        $(document).on('click', '.media-lang a', function() {

        	var mediaLang = $(this);

            KTApp.block('.blockui-media', {
                overlayColor: '#000000',
                opacity: '0.3',
                state: 'success',
                message: 'Chargement...'
            ...
            $.ajax({
                type: "POST",
                url: mediaLang.closest('ul').attr('data-url'),
                data: {
                	id: mediaLang.closest('form').find('input[name="media_id"]').val(),
                	lang: mediaLang.attr('data-lang'),
                },
                success: function (data) {
                    ...
                    if (data) {
						...
						$('#modalEditMedia input[name="media_lang"]').val(mediaLang.attr('data-lang'));

						KTApp.unblock('.blockui-media');
                    }
    ...
    var initInfiniteScroll = function() {
        ...
        function loadMedias(element) {

            if (isLoadingData || element.find('.medias-evenodd').hasClass('isLoadingData'))
                return;
            isLoadingData = true;

            element.find('.spinner').show();
            if (KTCardDraggable.length) {
	            KTCardDraggable.destroy();
	            swappable = KTCardDraggable.init();
	        }

            var mediaElem = element.find('.medias-evenodd');
            var requestUrl = mediaElem.attr('data-medialist-url');
            var offset = mediaElem.attr('data-offset');
            var draggable = mediaElem.hasClass('draggable-zone') ? 1 : 0;
            
            if ($('.masonry-layout').length) {
            	mediaElem.animate({ opacity: 0 }, 100);
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        var data = JSON.parse(data);
		                mediaElem.append(data).imagesLoaded().then(function(){
		                	...
		                	if ($('.masonry-layout').length) {
						    	...
						    	$("html, body").stop(true, false).animate({ scrollTop: $(".more-medias").offset().top }, 0);
						    }
				        });

                    	var limit = draggable ? 20 : 50;
                    	mediaElem.attr('data-offset', parseInt(offset) + limit);
    ...
    var initComposer = function() {
    	...
    	$('select').on('select2:select', function (evt) {
            if (evt.params !== undefined && evt.params.data !== undefined) {
        ...
        $(document).find('select').each(function() {
        	if ($(this).closest('.block').length && !$(this).closest('.block').hasClass('block-widget')) {
	            ...
	            $(this).select2({
	                placeholder: 'Selectionner un élément',
                    allowClear: $(this).hasClass('no-clear') ? false : true,
	                maximumSelectionLength: $(this).data('max-items') ? $(this).data('max-items') : 0,
	                debug: true, 
				    language: {
				        maximumSelected: function (e) {
				            return "Vous ne pouvez pas sélectionner plus de " + e.maximum + "élément";
				        }
				    }
	            });
        ...
        if ($('.block-anchor').length) {
        	$('.block-anchor').each(function() {
        		if (!$(this).hasClass('block-widget'))
        			loadAnchorBlocks($(this));
        	})
        }

        function loadAnchorBlocks(blockWidget) {

        	var blockCount = 1;
        	$('.composer').find('.block').each(function() {
        		if (!$(this).hasClass('block-widget') && !$(this).hasClass('block-anchor')) {
            		var currentBlock = $(this);
            		blockWidget.find('select').each(function() {
            			var selected = $(this).attr('data-value') == currentBlock.attr('data-block') + blockCount ? 'selected' : '';
            			$(this).append('<option value="' + currentBlock.attr('data-block') + blockCount + '" ' + selected + '>Bloc #' + blockCount + ' - ' + currentBlock.find('.block-unit > h3').text() + '</option>');
            		});
            		blockCount++;
            	}
        	});
        ...
        function reloadAnchorData() {

        	$('.block-anchor').each(function() {

        		if (!$(this).hasClass('block-widget')) {
	        		var currentSelect = $(this).find('select.select2');
	        		currentSelect.select2('destroy');
					currentSelect.find('option').remove().end().append('<option label="Label"></option>');
        ...
        $(document).on('click', '.add-block > a', function() {
            if ($(this).data('block') != 'block-faq' || (!$('.block-faq:not(.block-widget) .tree-faq').length && $(this).data('block') == 'block-faq')) {

                var blockWidget;
                if ($(this).closest('.add-block').hasClass('main-add')) {
                    blockWidget = $('.block-widget.'+$(this).data('block')).clone().appendTo('.composer > .row').removeClass('block-widget');
                } else {
                    blockWidget = $('.block-widget.'+$(this).data('block')).clone().insertAfter($(this).closest('.col-lg-12')).removeClass('block-widget');
                }
                updateBlockIdx();

                // Anchor block load blocks before select2 init
                if ($(this).data('block') == 'block-anchor') {
                	loadAnchorBlocks(blockWidget);
                } else if ($('.block-anchor').length) {
                	// If new block added and not an anchor block
    	        	reloadAnchorData();
    	        }
                        ...
                        if (dataInit[i] == 'select2') {
                            blockWidget.find('select').each(function() {
    	                        ...
    	                        $(this).select2({
    							    ...
    							    language: {
    							        maximumSelected: function (e) {
    							            return "Vous ne pouvez pas sélectionner plus de " + e.maximum + " éléments";
    							        }
    							    }
    	                        });
    					    	$(this).on('select2:select', function (evt) {
                ...
                alert('Un seul bloc FAQ par contenu CMS !');
            }
        });
        ...
        $(document).on('click', '.move-top', function() {
        	var prev = $(this).closest('.block').prev();
            if (prev.hasClass('block')) {
            	var elem = $(this).closest('.block');
            	animatedMove(elem, prev, 2);
                //$(this).closest('.block').insertBefore($(this).closest('.block').prev());
            }
        });

        $(document).on('click', '.move-bottom', function() {
        	var next = $(this).closest('.block').next();
            if (next.hasClass('block')) {
            	var elem = $(this).closest('.block');
            	animatedMove(elem, next, 1);
                //$(this).closest('.block').insertAfter($(this).closest('.block').next());
            }
        });

        function updateBlockIdx() {
            ...
            var idx = 0;
           	$('.block').not('.block-widget').each(function() {
           		idx++;
        ...
        function animatedMove($fstElem, $scdElem, dirflag) {
			...
			$fstElem.css({
				transform: 'translateY(' + fstdir + ($scdElem.height() + 64) + 'px)',
				transition: 'transform 0.4s'
			})
			$scdElem.css({
				transform: 'translateY(' + scddir + ($fstElem.height() + 64) + 'px)',
				transition: 'transform 0.4s'
			})
			setTimeout(function() {
				$fstElem.css({
        ...
        $(document).on('click', '.link-ddl', function() {
        	$(this).closest('.input-group-prepend').find('.btn').text($(this).text());
        	$(this).closest('.input-group').find('input').attr('placeholder', $(this).attr('data-placeholder'));
        });

        $(document).on('click', '.update-composer', function(e) {

            e.preventDefault();

            let jsonComposer = [];
            let idx = 0;
            $('.composer').find('.block').not('.block-widget').each(function() {
                ...
                jsonComposer[idx] = blockData;
                idx++;
            });

            $('#contentComposer').val(JSON.stringify(jsonComposer));
        ...
        if ($('#kt_uppy_2').length) {
            ...
            uppyDashboard.on('complete', result => {
                ...
                for (var i=0; i<result.successful.length; i++) {
                    ...
                    newImage.attr('title', result.successful[i].response.body.title);
                    $('[data-toggle="tooltip"]').tooltip({
                        trigger: 'hover',
                        template: '<div class="tooltip tooltip-dark" role="tooltip">\
                                <div class="arrow"></div>\
                    ...
                    var responseBody = result.successful[i].response.body;
                    newImage.attr('data-id', responseBody.id);
                    newImage.find('.action-btn > a').attr('data-media-id', responseBody.id);
                    newImage.find('.action-btn > a.edit-media').attr('data-media-src', newImage.attr('data-media-url') + responseBody.path);

                    switch (result.successful[i].extension) {
                		case 'jpg':
                		case 'png':
                		case 'gif':
                		case 'svg':
                    		newImage.find('.overflow-image img').removeClass('d-none').attr('src', newImage.attr('data-media-url') + responseBody.path);
                    		break;
    ...
    var initMenus = function() {
        ...
        $(document).on('click', '.save-menus', function() {
            ...
            let inputVal = $(this).closest('.card-body').find('input[name="menus"]');

            // AJAX
            $.ajax({
                type: "POST",
                url: inputVal.data('url'),
                data: {menus: inputVal.val()},
                success: function (data) {
    ...
    var initBlockMenu = function() {

        $(document).on('click', 'nav.block-menu a', function() {

		    var scrollAnchor = $(this).attr('data-scroll');
		    var scrollIdx = $(this).attr('data-scroll-idx');
			var scrollPoint = $('.composer div[data-block="' + scrollAnchor + '"][data-idx="' + scrollIdx + '"]').not('.block-widget').offset().top - 90;
			...
			$(window).scroll(function() {
			    ...
			    if (windscroll >= $('.composer').offset().top) {
			        ...
			        $('.composer div.block').each(function(i) {
			            if ($(this).offset().top <= windscroll + 200) {
			            	var activeMenu = $(document).find('nav.block-menu a[data-scroll="' + $(this).data('block') + '"][data-scroll-idx="' + $(this).data('idx') + '"]');
			            	if (activeMenu.length) {
			                	$(document).find('nav.block-menu a.active').removeClass('active');
			        ...
			        $('nav.block-menu').removeClass('d-block');
			        $(document).find('nav.block-menu a.active').removeClass('active');
			        $(document).find('nav.block-menu a:first').addClass('active');
			    }
    ...
    var initUpdates = function() {
        $(document).on('click', '.load-updates', function() {

        	var loadUpdates = $(this);

            KTApp.block('.blockui-updates', {
                overlayColor: '#000000',
                opacity: '0.3',
                state: 'success',
                message: 'Chargement...'
            ...
            $.ajax({
                type: "POST",
                url: loadUpdates.attr('data-url'),
                data: {
                	model: loadUpdates.attr('data-model'),
                	modelId: loadUpdates.attr('data-id'),
                	offset: loadUpdates.attr('data-offset'),
                },
                success: function (data) {
                    ...
                    if (data) {
                        ...
                        var data = JSON.parse(data);
                        $('.blockui-updates .timeline').append(data);
                        var loadCount = $(document).find('.load-count');
                        loadUpdates.attr('data-offset', parseInt(loadUpdates.attr('data-offset')) + parseInt(loadCount.attr('data-total')));
                        loadCount.remove();
                        if (parseInt(loadCount.attr('data-total')) < 5)
                        	loadUpdates.hide();
						
						KTApp.unblock('.blockui-updates');
                    }
...
var KTLayoutSearch = function() {
    ...
    var _processSearch = function() {
    	...
    	// show/hide more medias btn
    	if (_input.value.length > 1) {
    		$('#mediaContent .more-medias').hide();
    		mediaElem.addClass('isLoadingData');
    	} else {
    		$('#mediaContent .more-medias').show();
    		mediaElem.removeClass('isLoadingData');
    	}
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data) {
	                ...
	                $('.medias-evenodd .draggable').each(function() {
	                	$(this).remove();
	                });
